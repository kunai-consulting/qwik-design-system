...
### Custom Component with Indexing

```tsx
export const MyListRoot = component$(() => {
  const itemCount = 0;
  
  useContextProvider(myListContextId, { itemCount });
  
  return <ul><Slot /></ul>;
});

export const MyListItem = component$((props) => {
  const context = useContext(myListContextId);
  
  const index = useConstant(() => {
    const idx = context.itemCount;
    context.itemCount++;
    return idx;
  });

...
# Date Input
A customizable date input that allows users to enter dates in various formats using the keyboard.
<Showcase name="hero" />
...
- `<DateInput.Field>` groups the segments for proper accessibility and handles input, output, and state for an individual date value
...
#### Separators
The Field component accepts a `separator` prop, which then gets automatically inserted between the segments of the input.
```tsx
...
This can be useful when you have two or more Date Field components under the same root.
...
### Multiple dates
The Date Input can be composed with multiple `<DateInput.Field>` components to create a multi-date input.
You could use multiple `<DateInput.Root>` components, but if you have multiple dates that are related, with a single label, 
you can use a single `<DateInput.Root>` component with multiple `<DateInput.Field>` components,
...
To use the Date Input in a form, include the `<DateInput.HiddenInput>` component within the `<DateInput.Field>` component.
```tsx
<DateInput.Root>
  {/* ... */}
  <DateInput.Field>
...
Note that the `<DateInput.HiddenInput>` must be slotted within a `<DateInput.Field>` component in order to properly access the date value from the entry's context.

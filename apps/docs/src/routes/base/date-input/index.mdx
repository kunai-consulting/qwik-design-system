...
- `<DateInput.DateEntry>` groups the segments for proper accessibility and handles input, output, and state for an individual date value 
...
### Multiple dates
The Date Input can be composed with multiple `<DateInput.DateEntry>` components to create a multi-date input.
You could use multiple `<DateInput.Root>` components, but if you have multiple dates that are related, with a single label, 
you can use a single `<DateInput.Root>` component with multiple `<DateInput.DateEntry>` components, 
...
To use the Date Input in a form, include the `<DateInput.HiddenInput>` component within the `<DateInput.DateEntry>` component.
```tsx
<DateInput.Root>
  {/* ... */}
  <DateInput.DateEntry>
...
Note that the `<DateInput.HiddenInput>` must be slotted within a `<DateInput.DateEntry>` component in order to properly access the date value from the entry's context.

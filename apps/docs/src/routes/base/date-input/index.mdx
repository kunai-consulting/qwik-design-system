...
#### Separators
You can use the `<DateInput.Separator>` component to insert any sort of separator between or around the segments. 
The component accepts a `separator` prop to set the separator text:
```tsx
<DateInput.Separator separator="/" />
```
Alternatively, you can project custom content into the component's Slot:
```tsx
<DateInput.Separator>
  <MySeparatorIcon />
</DateInput.Separator>
```
Note that projected content will be ignored if the `separator` prop is provided.

Example using a custom separator icon:

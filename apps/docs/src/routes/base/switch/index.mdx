...
- Error state handling with `isError` prop
...
- `isError` - Controls error state display
#### Error State Management
The Switch supports error state handling through:
- The `isError` prop to indicate an error state
- The `ErrorMessage` component to display error messages
...
- Error messages require both `isError` prop and `ErrorMessage` component

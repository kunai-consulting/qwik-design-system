...
Dropdowns now support nested submenus for more complex menu structures. Use `Dropdown.Submenu` to wrap a submenu, `Dropdown.SubmenuTrigger` to provide the trigger item, and `Dropdown.SubmenuContent` for the submenu's content.

<Showcase name="submenu" />
This example demonstrates:
- Creating a submenu inside a dropdown
- Using `Dropdown.SubmenuTrigger` to open the submenu
...
This example demonstrates:
- Using `Dropdown.ContextTrigger` to create a right-click context menu
...
<Dropdown.Root>
  ...
  <Dropdown.Trigger />        {/* Click trigger */}
  <Dropdown.ContextTrigger /> {/* Right-click trigger */}
...
- `ContextTrigger`: Area that responds to right-clicks to open the dropdown as a context menu

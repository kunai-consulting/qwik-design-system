...
export const CheckboxRoot = component$<MixedStateCheckboxProps>((props) => {
  const checked = useBoundSignal(props['bind:checked'], false);
...
export const TwoStateCheckbox = component$<TwoStateCheckboxProps>((props) => {
  ...
  const { checked } = useCheckboxState(props['bind:checked']);
  const checklistID = useSignal<string | undefined>(props.id);
  useContextProvider(CheckboxContext, checked);
  return (
    <TwoStateCheckboxBehavior
      bind:checked={checked}
      tabIndex={0}
      role="checkbox"
      aria-checked={getAriaChecked(checked.value)}
      {...props}
...
export const MixedStateCheckbox = component$<MixedStateCheckboxProps>(
  (props) => {
    console.log('MixedStateCheckbox');
    ...
    console.log(
      'MixedStateCheckbox checklistContext.checklistSig.value',
      checklistContext.checklistSig.value
    );
    console.log('ariaControlsStrg', ariaControlsStrg);
    ...
    const changeChecklistSig = $(() => {
      if (appliedSig.value !== true) {
        ...
        for (const sig of childCheckboxes.value) {
          sig.value = true;
        }
...
const TwoStateCheckboxBehavior = component$<TwoStateCheckboxBehaviorProps>(
  (props) => {
    ...
    // this logic is duplicated thrice, make into hook pls
    const handleClick = $(() => {

...
### Native HTML Dialog Note

The native `<dialog>` element has a `closedBy` property with values like `"any"`, `"closerequest"`, `"none"` but these are not intuitive (not fully supported by all browsers). Our API uses more explicit boolean props like `closeOnOutsideClick` for better developer experience.

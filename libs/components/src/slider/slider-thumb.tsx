...
interface ThumbProps extends PropsOf<"div"> {
  type?: ThumbType;
}

export const SliderThumb = component$((props: ThumbProps) => {
  ...
  const percentage = parentContext.mode.value === 'single'
    ? ((parentContext.value.value - parentContext.min.value) /
    (parentContext.max.value - parentContext.min.value)) * 100
    : ((type === 'start' ? parentContext.startValue.value : parentContext.endValue.value - parentContext.min.value) /
    (parentContext.max.value - parentContext.min.value)) * 100;
  ...
  const onPointerUp$ = $(async (event: PointerEvent) => {
    ...
    if (rect) {
      const newValue = await calculateValue(event.clientX, rect);
      await setValue(newValue, parentContext.mode.value === 'range' ? type : undefined, true);
    }
  });
  ...
  const onKeyDown$ = $(async (event: KeyboardEvent) => {
    ...
    const step = event.shiftKey ? parentContext.step.value * 10 : parentContext.step.value;
    let newValue: number;

    if (parentContext.mode.value === 'single') {
      newValue = parentContext.value.value;
    } else {
      newValue = type === 'start' ? parentContext.startValue.value : parentContext.endValue.value;
    }

    switch (event.key) {
      case "ArrowRight":
      case "ArrowUp": {
        const maxValue = parentContext.mode.value === 'range' && type === 'start'
      ...
      case "ArrowLeft":
      case "ArrowDown": {
        const minValue = parentContext.mode.value === 'range' && type === 'end'
      ...
      case "Home":
        newValue = parentContext.mode.value === 'range' && type === 'end'
          ? parentContext.startValue.value
          : parentContext.min.value;
        break;
      case "End":
        newValue = parentContext.mode.value === 'range' && type === 'start'
          ? parentContext.endValue.value
          : parentContext.max.value;
        break;
      default:
  ...
  return (
    <div
      ...
      aria-valuemin={parentContext.mode.value === 'range' && type === 'start' ? parentContext.min.value : parentContext.startValue.value}
      aria-valuemax={parentContext.mode.value === 'range' && type === 'end' ? parentContext.max.value : parentContext.endValue.value}
      aria-valuenow={parentContext.mode.value === 'single' ? parentContext.value.value : (type === 'start' ? parentContext.startValue.value : parentContext.endValue.value)}

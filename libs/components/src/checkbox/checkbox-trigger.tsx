import { $, component$, type PropsOf, Slot, sync$, useComputed$, useContext } from "@builder.io/qwik";
...
export const CheckboxTrigger = component$((props: PublicCheckboxControlProps) => {
    ...
    return (<button id={triggerId} ref={context.triggerRef} type="button" role="checkbox" aria-checked={`${context.isCheckedSig.value}`} aria-describedby={describedByLabels ? describedByLabels.value : undefined} aria-invalid={context.isErrorSig.value} disabled={context.isDisabledSig.value} 
    // Indicates whether the checkbox trigger is disabled
    data-disabled={context.isDisabledSig.value ? "" : undefined} onKeyDown$={[handleKeyDownSync$, props.onKeyDown$]} onClick$={[handleClick$, props.onClick$]} 
    // Indicates whether the checkbox trigger is checked
    data-checked={context.isCheckedSig.value && context.isCheckedSig.value !== "mixed"
            ? ""
            : undefined} 
    ...
    data-qds-checkbox-trigger {...props}>
      <Slot />
    </button>);
});

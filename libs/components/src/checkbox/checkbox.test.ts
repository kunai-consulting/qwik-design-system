...
import { createTestDriver } from "./checkbox.driver.ts";
...
test.describe("state", () => {
	...
	test(`GIVEN a checkbox with a disabled prop
        ...
        THEN the checkbox trigger should reflect the new state`, async ({
		page,
	}) => {
		const d = await setup(page, "disabled");
...
test.describe("a11y", () => {
	...
	test(`GIVEN a checkbox that is initially mixed
        WHEN the checkbox is rendered
        THEN the trigger should have aria-checked="mixed"`, async ({
		page,
	}) => {
...
`, async ({ page }) => {
		...
		await page.getByRole("button").last().click();
		await expect(
			page.getByText(`Submitted: { "terms": "checked" }`),
		).toBeVisible();
	});
});

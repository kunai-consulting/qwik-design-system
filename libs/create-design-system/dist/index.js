...
import { bgBlue, bgMagenta, bold, cyan, gray, green, magenta, red, reset, white } from "kleur/colors";
...
export function resolveAbsoluteDir(dir) {
...
export const clearDir = async (dir) => {
    const files = await fs.promises.readdir(dir);
    return await Promise.all(files.map((pathToFile) => fs.promises.rm(join(dir, pathToFile), { recursive: true })));
};
export function fileGetContents(file) {
    if (!fs.existsSync(file)) {
        throw new Error(`File ${file} not found`);
    }
    return fs.readFileSync(file, { encoding: "utf8" }).toString();
}
export function filePutContents(file, contents) {
    return fs.writeFileSync(file, contents, { encoding: "utf8" });
}
export function fileReplaceContents(file, search, replace) {
...
export const installDependencies = async (cwd) => {
    await $pm("install", cwd);
};
export function parseArgs(args) {
    const parsedArgs = yargs(args)
        .strict()
        .command("* <project> [adapter]", "Create a new project powered by QwikDev/astro", (yargs) => {
        return yargs
            .positional("project", {
            type: "string",
            ...
            .example("npm create @qwikdev/astro@latest", "Create a project in interactive mode")
            .example("npm create @qwikdev/astro@latest ./qwik-astro-app node", "Create a project in commande mode")
            .example("npm create @qwikdev/astro@latest ./qwik-astro-app node --it", "Create a project in interactive command mode")
            .usage("npm create @qwikdev/astro [project] [adapter] [...options]");
    })
        .alias("h", "help").argv;
    return parsedArgs;
}
export async function createProject(config, defaultProject) {
    try {
        intro(`Let's create a ${bgBlue(" QwikDev/astro App ")} ✨`);
        const packageManager = getPackageManager();
        const projectAnswer = config.project !== defaultProject || !config.it
            ? config.project
            : (await text({
        ...
        const preferBiome = config.no && !config.biome
            ? false
        ...
        const outDir = resolveAbsoluteDir(projectAnswer.trim());
        log.step(`Creating new project in ${bgBlue(` ${outDir} `)} ... 🐇`);
        if (fs.existsSync(outDir) && fs.readdirSync(outDir).length > 0) {
            const force = config.no && !config.force
                ? false
                : (config.yes && config.force !== false) ||
                    config.force ||
                    (config.it &&
                        (await confirm({
                            message: `Directory "./${resolveRelativeDir(outDir)}" already exists and is not empty. What would you like to overwrite it?`,
                            initialValue: true
                        })));
            if (force) {
                if (!config.dryRun) {
                ...
                log.info(`Please either remove this directory, choose another location or run the command again with '--force | -f' flag.`);
                cancel();
                process.exit(1);
            }
        }
        ...
        const addCIWorkflow = config.no && !config.ci
            ? false
        ...
        if (addCIWorkflow && !config.dryRun) {
            const starterCIPath = join(__dirname, "..", "stubs", "workflows", `${["npm", "yarn", "pnpm", "bun"].includes(packageManager) ? packageManager : "npm"}-ci.yml`);
            const projectCIPath = join(outDir, ".github", "workflows", "ci.yml");
            cpSync(starterCIPath, projectCIPath, { force: true });
        }
        const runInstall = config.no && !config.install
            ? false
        ...
        const initGit = config.no && !config.git
            ? false
        ...
        if (isCwdDir) {
            ...
            outString.push(`🦄 ${bgMagenta(" Success! ")} ${cyan("Project created in")} ${bold(magenta(relativeProjectPath))} ${cyan("directory")}`);
        }
        outString.push("");

...
export async function createProject(
  ...
  try {
    ...
    if (
      typeof projectAnswer === "symbol" ||
      isCancel([projectAnswer, packageManager])
    ) {
      panicCanceled();
    }
      ...
      if (fs.existsSync(join(outDir, ".git"))) {
        ...
        try {
          if (!config.isDryRun) {
            ...
            res.push(
              await $("git", ["commit", "-m", "Initial commit 🎉"], outDir)
                .install
            );
    ...
    console.error(
      "An error occurred during QwikDev/astro project creation:",
      err
    );
    process.exit(1);
  }
